...
public class UserFaceService {
    ...
    private static final Set<String> ALLOWED_FORMATS = Set.of("FACE_TEMPLATE_V1");
    ...
    public void create(String publicId, byte[] bytes, String format) {
        ...
        try {
            ...
            throw Errors.conflict("UserFace already exists", Map.of("publicId", publicId));
        }
    }
    ...
    public void update(String publicId, byte[] bytes, String format) {
        ...
        UserFace face = userFaceRepository.findForUpdateByUserPublicId(publicId)
                .orElseGet(() -> {
                    User user = userRepository.findByPublicId(publicId)
                            .orElseThrow(() -> Errors.notFound("User not found", Map.of("publicId", publicId)));
    ...
    public UserFace get(String publicId) {
        return userFaceRepository.findByUser_PublicId(publicId)
                .orElseThrow(() -> Errors.notFound("UserFace not found", Map.of("publicId", publicId)));
    }

    @Transactional
    public void delete(String publicId) {
        if (!userFaceRepository.existsByUser_PublicId(publicId)) {
            throw Errors.notFound("UserFace not found", Map.of("publicId", publicId));
        }
    ...
    private static void validate(byte[] bytes, String format) {
        if (bytes == null || bytes.length == 0) {
            throw Errors.validation("Face template is empty", Map.of("size", 0));
        }
        ...
        if (bytes.length > MAX_BYTES) {
            throw Errors.validation(
                    ...
                    Map.of("maxBytes", MAX_BYTES, "actualBytes", bytes.length)
            );
        }
        ...
        if (!ALLOWED_FORMATS.contains(f)) {
            throw Errors.validation(
                    ...
                    Map.of("allowed", ALLOWED_FORMATS, "provided", f)
            );
        }
    }
    ...
    public boolean exists(String publicId) {
        return userFaceRepository.existsByUser_PublicId(publicId);
    }
